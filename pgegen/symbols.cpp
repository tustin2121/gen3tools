...
struct ExpressionToken {
	...
	TokenType type;
	long value;
	vector<ExpressionToken*> subTokens;
};
...
private:
	...
	const unordered_map<string, uint64_t> m_symbolMap; //unowned
...
public:
	ExpressionParser(string expr, unordered_map<string, uint64_t> map) 
	...
	ExpressionToken* parseGroup() 
	{
		...
		while (m_ptr < m_expr + m_exprLength)
		{
			...
			if (*m_ptr == '(')
			{
				...
				if (m_errorState != nullptr) return;
				group->subTokens.push_back(workingToken);
				workingToken = nullptr;
				goto loop;
			}
